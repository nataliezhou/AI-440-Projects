...
def minimax(side, board, flags, depth):
    ...
    '''
    def minimax_helper(side, board, flags, depth, tree):
        if depth == 0:
          return (evaluate(board), [], {})
        else:
          ...
          for i in generateMoves(side, board, flags):
              newside, newboard, newflags = makeMove(side, board, i[0], i[1], flags, i[2])
              value, newMoveList, newMoveTree = minimax_helper(newside, newboard, newflags, depth-1, tree)
              if value < bestValue and side or (value > bestValue and not side):
                  moveList = [i] + newMoveList
                  bestValue = value
              moveTree[encode(*i)] = newMoveTree
          return (bestValue, moveList, moveTree)
    return minimax_helper(side, board, flags, depth, {})
...
def alphabeta(side, board, flags, depth, alpha=-math.inf, beta=math.inf):
    ...
    if depth == 0:
      ...
      if len(moves) == 0:
          return (evaluate(board), [], {})
      for i in moves:
          side1, board1, flags1 = makeMove(
              side, board, i[0], i[1], flags, i[2])
          value1, moveList1, moveTree1 = alphabeta(
              side1, board1, flags1, depth - 1, alpha, beta)
          moveTree[encode(*i)] = moveTree1
          if (side and value1 < bestValue) or (not side and value1 > bestValue):
              bestValue = value1
              moveList = [i] + moveList1
          if not side:
