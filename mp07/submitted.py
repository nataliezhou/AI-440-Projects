...
def standardize_variables(nonstandard_rules):
    ...
    variables = []
    for ruleid, rule in standardized_rules.items():
        for antecedent in rule['antecedents']:
            for i in range(len(antecedent)):
                if antecedent[i] == 'something':
                    antecedent[i] = ruleid
                    variables.append(ruleid)
        for i in range(len(rule['consequent'])):
            if rule['consequent'][i] == 'something':
                rule['consequent'][i] = ruleid
...
def unify(query, datum, variables):
    ...
    datum1 = copy.deepcopy(datum)
    subs = {}
    for i in range(len(query)):
        if unification[i] in variables:
            if unification[i] in subs:
                if subs[unification[i]] != datum1[i]:  
                    return None, None
            else:
                subs[unification[i]] = datum1[i]
                for i in range(len(unification)):
                  if unification[i] in subs:
                      unification[i] = subs[unification[i]]
        
    for i in range(len(datum1)):
      if datum1[i] in subs:
          if subs[datum1[i]] != unification[i]:  
              return None, None
      elif unification[i] not in variables and datum1[i] in variables: 
          subs[datum1[i]] = unification[i]
          for i in range(len(unification)):
              if unification[i] in subs:
                  unification[i] = subs[unification[i]]
...
def apply(rule, goals, variables):
    ...
    for i in range(len(goals)):
      unification, subs = unify(rule['consequent'], goals[i], variables)
      if unification != None:
        newrule = copy.deepcopy(rule)
        newrule['consequent'] = unification
        for j in range(len(newrule['antecedents'])):
          for k in range(len(newrule['antecedents'][j])):
            if newrule['antecedents'][j][k] in subs:
              newrule['antecedents'][j][k] = subs[newrule['antecedents'][j][k]]
        applications.append(newrule)
        newgoals = copy.deepcopy(goals)
        newgoals.remove(goals[i])
        for j in newrule['antecedents']:
          newgoals.append(j)
...
def backward_chain(query, rules, variables):
    ...
    while len(frontier) > 0:
      goals = frontier.pop(0)
      if len(goals) == 0:
        proof = []
        break
      for i in rules:
        applications, newgoals = apply(rules[i], goals, variables)
        for j in range(len(applications)):
          frontier.append(newgoals[j])
          if proof == None:
            proof = [applications[j]]
          else:
            proof.append(applications[j])
    return proof
