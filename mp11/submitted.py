...
class q_learner():
    ...
    def report_exploration_counts(self, state):
        ...
        '''
       # Check to see how many times each action has been explored in this state.
        explored_count = self.N[state[0], state[1], state[2], state[3], state[4], :]
    ...
    def choose_unexplored_action(self, state):
        ...
        if np.all(explored_count >= self.nfirst):
            ...
            action = np.random.choice(np.where(explored_count < self.nfirst)[0])
    ...
    def report_q(self, state):
        ...
        '''
        # Report the current Q values for the given state.
        Q = self.Q[state[0], state[1], state[2], state[3], state[4], :]
        return Q
    ...
    def q_local(self, reward, newstate):
        ...
        # The update to Q estimated from a single step of game play:
        # reward plus gamma times the max of Q[newstate, ...].
        Q_local = reward + self.gamma * np.max(self.report_q(newstate))
        return Q_local
    ...
    def learn(self, state, action, reward, newstate):
        ...
        # Update the internal Q-table on the basis of an observed
        # state, action, reward, newstate sequence.
        self.Q[state[0], state[1], state[2], state[3], state[4], action] += self.alpha * (self.q_local(
            reward, newstate) - self.Q[state[0], state[1], state[2], state[3], state[4], action])
    ...
    def exploit(self, state):
        ...
        '''
        # action is either -1, 0, or 1
        # Q is the Q-value of the selected action
        Q = self.report_q(state) # q values for the current state
        action = np.argmax(Q)
        if action == 2:
            action = -1
    ...
    def act(self, state):
        ...
        # Otherwise, with probability epsilon, choose an action uniformly at random.
        # Otherwise, exploit.
        if np.min(self.N[state[0], state[1], state[2], state[3], state[4], :]) < self.nfirst:
            action = self.choose_unexplored_action(state)
        elif np.random.rand() <= self.epsilon:
            action = np.random.randint(0, 3) - 1
        else:
            action, Q = self.exploit(state)
        return action
